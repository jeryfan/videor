...
async fn fetch_via_mobile_share(
    ...
    let title = item
        .get("desc")
        .and_then(|v| v.as_str())
        .map(|s| if s.trim().is_empty() { "抖音视频".to_string() } else { s.to_string() })
    ...
    if let Some(url_list) = item.pointer("/video/play_addr/url_list").and_then(|v| v.as_array()) {
        for (idx, url_val) in url_list.iter().enumerate() {
            if let Some(raw_url) = url_val.as_str() {
                let url = raw_url.replace("playwm", "play");
                let label = if idx == 0 { "原画" } else { &format!("线路 {}", idx + 1) };
    ...
    if formats.is_empty() {
        if let Some(url_list) = item.pointer("/video/download_addr/url_list").and_then(|v| v.as_array()) {
            for (idx, url_val) in url_list.iter().enumerate() {
...
fn extract_router_data(html: &str) -> Result<Value, String> {
    ...
    serde_json::from_str(json_str)
        .map_err(|e| format!("Failed to parse _ROUTER_DATA: {e}"))
}
